...
public class ItemController {
    ...
    public ApiResponse<ItemDTO.ItemResponseDTO> getItemDetails(@PathVariable Long itemId) {
        ItemDTO.ItemResponseDTO itemResponse = ItemConverter.toResponseDTO(itemService.showItem(itemId));
        return ApiResponse.onSuccess("물품조회 완료", itemResponse);
    }

    @GetMapping("/recent")
    @Operation(summary = "아이템 상세 조회", description = "사용자가 물품을 조회합니다.")
    public ApiResponse<List<ItemDTO.ItemResponseDTO>> getItemDetails(@ModelAttribute("recentItems") List<Object> recentItems) {
        List<ItemDTO.ItemResponseDTO> itemResponseList = recentItems.stream()
                .map(item -> {
                    Item recentItem = (Item) item; // RecentItem은 예시
                    Long itemId = recentItem.getId(); // 아이템 ID 가져오기
                    return ItemConverter.toResponseDTO(itemService.showItem(itemId)); // 아이템을 조회하여 DTO로 변환
                })
